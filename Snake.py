...
import random
import sys
#Для начала откроем нужные библиотеки.
...
class SnakeGame(QMainWindow):
    def __init__(self):
        ...
        self.sboard.start()
        self.show()
#Теперь создадим класс SnakeGame, который наследуется от QMainWindow
#Это даёт нам возможность использовать обьекты.
...
class Board(QFrame):
    ...
    WIDTHINBLOCKS = 60
    HEIGHTINBLOCKS = 40
#Этот класс позволяет нам создать поле.
#На этом поле будет производиться игра.
...
#Вверх
    ...
    def is_suicide(self): 
        for i in range(1, len(self.snake)):
            if self.snake[i] == self.snake[0]:
                ...
                self.timer.stop()
                self.update()
                #Завершение игры при поедании себя
    ...
    def is_food_collision(self):
        for pos in self.food:
            if pos == self.snake[0]:
                ...
                #Увеличение змейки при поедании 
    ...
    def drop_food(self):
        ...
        #Появление еды на карте, но не на змейке 
